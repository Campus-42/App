...
export async function uploadVideo(paths, uri) {
  const path = paths.join("/");
  const storageRef = storage.ref(path);
  ...
  return storageRef
    ...
    .catch((err) => {
      analytics.error(err, "Firebase/functions.js", "uploadVideo()");
      throw err;
    });
}
...
export async function parseEventData(docData, id) {
  return {
    ...
    search_index: [],
    date: {
      end: docData.date.end.toDate(),
      start: docData.date.start.toDate(),
    },
    __type: "event",
  };
}
export async function parseStoryData(docData, id) {
  return {
    ...docData,
    id: id,
  };
