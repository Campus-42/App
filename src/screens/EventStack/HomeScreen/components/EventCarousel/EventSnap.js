...
export const EventSnap = (props) => {
  ...
  const participants = [
    ...new Set(
      (props.reduxEvent.participants || []).concat(
      ...
      >
        ...
        <View style={styles.rightContainer}>
          ...
          {participants.includes(auth.currentUser.uid) ? (
            <LinearGradient
              colors={["#c634f7", "#f7343a"]}
              start={{ x: 0, y: 0.3 }}
              end={{ x: 0.7, y: 1 }}
              style={styles.joinedTag}
            >
              <Ionicon
                name={"star"}
                size={GlobalStyle.Measurements.unit * 0.6}
                color={styles.joinedTagText.color}
              />
              <Text style={styles.joinedTagText}>Joined</Text>
            </LinearGradient>
          ) : participants.length > 0 || false ? (
...
export const styles = StyleSheet.create({
  ...
  joinedTag: {
    borderRadius: GlobalStyle.Measurements.marginHalf,
    paddingHorizontal: GlobalStyle.Measurements.marginQuarter,
