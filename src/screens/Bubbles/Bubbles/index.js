...
export class BubblesAndChannels extends React.Component {
  ...
  render() {
     ...
     * segment controller
     */
    const unreadBubbles = (this.state.bubbles || []).some(
      MessageFuncs.isConversationUnread,
    );
    const unreadConversation = (this.state.channels || []).some(
      MessageFuncs.isConversationUnread,
    );
  ...
  getBubbles = (refreshing = true, override = false) => {
    ...
    MessageFuncs.getBubbles(latestTimestamp)
      ...
      .finally(() => setTimeout(() => this.setState({refreshing: false}), 750));
  };
