...
export class BubbleFocus extends React.Component {
            ...
            {!this.disabled &&
              ...
              ((this.type == "channel" &&
                ...
                <TextInput
                  ...
                  showChannelOptions={console.log}
                />
              )}
  ...
  getConversation = (init = false) => {
    MessageFuncs.getBubbleFocusConversation(
      ...
      .then(({ validChannelUids }) => {
        this.validChannelUids = validChannelUids;
  ...
  getNewMessages = async () => {
    ...
    if (this.state.screenIndex == 0) {
      ...
      var ref = db.collection("bubbles").doc(this.id).collection("messages");
      ...
      ref
        ...
        .then((querySnapShot) => {
          ...
          querySnapShot.forEach(async (doc, index) => {
            ...
            if (this.type === "channel")
              messages = messages.filter((m) =>
                this.validChannelUids.includes(m.creator)
              );
            ...
            if (index == querySnapShot.size - 1) {
              ...
              this.setState({
                messages: filtered.concat(this.state.messages),
              });
            }
          });
        })
  ...
  send = async (text, type = "user", otherParams = {}, resend = false) => {
    ...
    const { __data, custom, image, system, video, id } = {
      ...{},
      ...otherParams,
    }; // prevent undefined otherParams
